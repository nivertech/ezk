...
init(Servers) ->
    ...
    ?LOG(1,"ankommende args: ~w",[Servers]),
    Mglkeiten = length(Servers),    
    Serverwahl = random:uniform(Mglkeiten),
    ?LOG(0,"Choose server ~w from ~w",[Serverwahl, Mglkeiten]),
    {Ip, Port, WantedTimeout} =  lists:nth(Serverwahl, Servers),
